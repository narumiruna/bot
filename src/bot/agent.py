...
def log_new_items(new_items: list[RunItem]) -> None:
    for new_item in new_items:
        if isinstance(new_item, MessageOutputItem):
            logfire.info("Message: {}", shorten_text(ItemHelpers.text_message_output(new_item)))
            ...
            if isinstance(new_item.raw_item, ResponseFunctionToolCall):
                logfire.info("Calling tool: {}({})", new_item.raw_item.name, new_item.raw_item.arguments)
        elif isinstance(new_item, ToolCallOutputItem):
            logfire.info("Tool call output: {}", shorten_text(str(new_item.raw_item["output"])))
        else:
            logfire.info("Skipping item: {}", new_item.__class__.__name__)
...
class AgentService:
    ...
    async def handle_message(
        ...
        if messages is None:
            messages = []
            logfire.info("No key found for {}", key)
